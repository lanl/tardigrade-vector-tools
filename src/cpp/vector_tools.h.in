...
namespace vectorTools{
    ...
    template< typename T >
    bool fuzzyEquals(const std::vector< std::vector< T > > &A, const std::vector< std::vector< T > > &B, double tolr=1e-6, double tola=1e-6){
        /*!
    ...
    //Utilities which require Eigen
        ...
        std::vector< T > matrixMultiply(const std::vector< T > &A, const std::vector< T > &B,
             ...
             * \f$ C_{ij} = A_{ik} B_{kj} \f$ if Atranspose = Btranspose = false
             * \f$ C_{ij} = A_{ki} B_{kj} \f$ if Atranspose = true, Btranspose = false
             * \f$ C_{ij} = A_{ik} B_{jk} \f$ if Atranspose = false, Btranspose = true
             * \f$ C_{ij} = A_{ki} B_{jk} \f$ if Atranspose = true, Btranspose = true
             *
